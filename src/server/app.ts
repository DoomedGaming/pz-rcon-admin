...
import { createAuth, createPlayerAuth, safeEqual } from './auth.js'
...
export interface CreateDashboardServerOptions {
  // Invoked after a secure-configuration save has been acknowledged; the
  // entry point stops its timers and exits so the supervisor restarts us.
  requestRestart: () => void
}

// Express 5 hops are only trusted when the operator opts in, e.g.
// DASHBOARD_TRUST_PROXY=1 behind a single reverse proxy.
...
export function createDashboardServer(options: CreateDashboardServerOptions) {
  const app = express()
  ...
  const playerLoginAttemptsByIp = new Map<string, { count: number; resetAt: number }>()
  ...
  function loginAttemptsExceeded(attempts: Map<string, { count: number; resetAt: number }>, key: string, limit: number, now: number): boolean {
    // Expired entries would otherwise accumulate from attacker-chosen keys.
    if (attempts.size > 1_000) {
      for (const [staleKey, entry] of attempts) {
        if (entry.resetAt <= now) attempts.delete(staleKey)
      }
    }
    const attempt = attempts.get(key)
    return Boolean(attempt && attempt.resetAt > now && attempt.count >= limit)
  }

  function recordFailedLoginAttempt(attempts: Map<string, { count: number; resetAt: number }>, key: string, now: number) {
    const attempt = attempts.get(key)
    attempts.set(key, {
  ...
  function requirePlayerUsername(request: Request, response: Response): string | undefined {
    const username = playerAuth.username(request)
    if (!username) response.status(401).json({ error: 'Player authentication required' })
    return username
  }
  ...
  if (appConfig.trustProxy) app.set('trust proxy', parseTrustProxy(appConfig.trustProxy))
  ...
  function scheduleConfigurationRestart(response: Response) {
    ...
    const beginRestart = () => {
      if (armed) return
      armed = true
      const restartTimer = setTimeout(() => options.requestRestart(), 750)
      restartTimer.unref()
    }
  ...
  app.get('/map-tiles/:mapId/:level/:tile', async (request, response) => {
    // Before first-run setup no dashboard password exists, so the bootstrap
    // auth check would otherwise treat every visitor as authenticated.
    if (setupRequired && !setupSaved) return response.status(503).end()
  ...
  app.post('/api/login', (request, response) => {
    ...
    const now = Date.now()
    if (loginAttemptsExceeded(loginAttempts, key, 8, now)) {
    ...
    if (!auth.login(String(request.body?.password ?? ''), response)) {
      recordFailedLoginAttempt(loginAttempts, key, now)
  ...
  app.post('/api/player/login', async (request, response) => {
    ...
    const now = Date.now()
    // The per-IP limit stops one source from spraying passwords across many
    // usernames — each attempt downloads the whitelist database over FTP.
    if (loginAttemptsExceeded(playerLoginAttempts, key, 8, now) || loginAttemptsExceeded(playerLoginAttemptsByIp, ipKey, 30, now)) {
      return response.status(429).json({ error: 'Too many attempts. Try again in a few minutes.' })
    }
    ...
    if (!canonicalUsername) {
      recordFailedLoginAttempt(playerLoginAttempts, key, now)
      recordFailedLoginAttempt(playerLoginAttemptsByIp, ipKey, now)
  ...
  app.get('/api/player/me', (request, response) => {
    const username = requirePlayerUsername(request, response)
    if (!username) return
  ...
  async function poll() {
    try {
      ...
      // A failed state save must not become an unhandled rejection that
      // brings down the process.
      console.warn(`Player poll failed: ${errorMessage(error)}`)
    }
  }
