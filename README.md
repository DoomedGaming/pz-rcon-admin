...
PZ_ADMIN_PUBLIC_URL=https://pz.example.com
...
RCON cannot validate player accounts. When enabled, the backend retrieves `db/servertest.db` from the configured FTP/FTPS host into a private temporary directory, finds the exact case-sensitive account in the `whitelist` table, verifies the Build 42 MD5-plus-bcrypt password representation, and immediately removes the database copy. The raw password, stored hash, and database are never returned to the browser or written to dashboard history.
...
The player API derives the username from a signed, HttpOnly session. Project Zomboid usernames are case-sensitive throughout the portal, so accounts such as `howop` and `Howop` retain separate records, settings, requests, and permissions. Every account starts as **User**, which can reach only its own portal data. A **Moderator** can open the staff overview, survivor registry, and moderation actions. Kick, ban, and whitelist removal require a reason in both the interface and API, and the reason is recorded in the administrator audit log. An **Admin** can additionally view that audit log and manage dashboard roles, server and world commands, live settings, configuration, and the raw RCON console. Dashboard roles are independent from Project Zomboid's in-game access levels.

Admins can remove an offline survivor from the local dashboard registry after typing the exact case-sensitive username. Removal clears that survivor's local observed history, telemetry, dashboard role, and theme, but deliberately leaves the Project Zomboid account, whitelist, save data, support-request history, and audit history unchanged. The dashboard creates a fresh User record if the survivor signs in or is observed again.
...
An optional Discord channel webhook can notify moderators about new requests, player and staff request replies, request claims and status changes, and successful kick, ban, or whitelist-removal actions. The integration is deliberately called from those moderation workflows rather than from the audit store, so authentication, telemetry, raw console commands, server settings, and other administrator audit entries never reach Discord. Messages suppress Discord mentions even when player-written text contains `@everyone` or a user/role mention. Request notifications include a link to `/mod?request=<request-id>`; an authenticated staff member who follows it sees that exact request in a focused dialog, including its conversation and moderation controls. Create a channel webhook in Discord, then save its URL and the dashboard's public HTTPS origin under **Configuration → Discord moderator notifications**. The webhook URL is treated as a secret, encrypted at rest, and never returned to the browser. The public Admin URL is required with the webhook so request links never rely on an untrusted request header.
...
The basemap is selected from the Project Zomboid Map Project's official top-down **Build 42.19** and **Build 42.20 stable** image pyramids using the authoritative server build reported by the telemetry companion. Both supported maps use the same 19,968 by 16,128 coordinate space, so game `x` and `y` map directly to image coordinates while `z` is shown as the reported floor. The dashboard does not guess when the build is missing or unsupported; it waits for a supported server version instead of showing potentially incorrect terrain.

Browsers never contact the external tile host directly. Map images are requested through the dashboard's same-origin `/map-tiles` gateway, which requires either a valid player session or a valid administrator session. The gateway accepts only the two configured map IDs, their matching WebP or JPEG format, allowed zoom levels, and in-range tile coordinates; it is not a general-purpose proxy. Versioned gateway paths keep browser caches for the two maps separate. The gateway forwards no usernames, player records, or dashboard credentials to the map provider.
...
**Custom-map limitation:** the supported basemaps cover the official Build 42.19 and Build 42.20 worlds only. They do not load arbitrary Workshop or private mod-map tiles. A survivor on added or replacement terrain may therefore appear over inaccurate vanilla imagery when coordinates overlap, or be reported as outside map coverage when they do not. Supporting a custom map requires an intentionally configured tile pyramid and coordinate offset for that map; the dashboard does not guess either value.
